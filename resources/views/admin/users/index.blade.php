<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('users.create') }}" class="btn-accent text-white font-bold px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all inline-flex items-center gap-2">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-ink">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">Roles</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($users as $user)
                                    <tr class="hover:bg-cream/50 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-ink font-mono">{{ $user->id }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-semibold text-ink">{{ $user->name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-ink/80">{{ $user->email }}</div>
                                        </td>
                                        <td class="px-6 py-4">
                                                ...
                                                @if($user->roles->count() > 0)
                                                    <div class="flex flex-wrap gap-1">
                                                        @foreach($user->roles as $role)
                                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-accent/20 text-accent border border-accent/30">
                                                                {{ $role->name }}
                                                            </span>
                                                        @endforeach
                                                    </div>
                                                @else
                                                    <span class="text-ink/40">No roles</span>
                                                @endif
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-3">
                                                <a href="{{ route('users.edit', $user) }}" class="text-accent hover:text-accentlight font-semibold transition-colors inline-flex items-center gap-1">
                                    ...
                                    <tr>
                                        <td colspan="6" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center gap-3">
                                                ...
                                                <p class="text-ink/60 text-lg">No users registered.</p>
                                                <a href="{{ route('users.create') }}" class="btn-accent text-white font-bold px-6 py-3 rounded-xl inline-flex items-center gap-2 mt-2">
