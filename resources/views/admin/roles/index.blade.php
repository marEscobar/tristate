<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('roles.create') }}" class="btn-accent text-white font-bold px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all inline-flex items-center gap-2">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-ink">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">Permissions</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($roles as $role)
                                    <tr class="hover:bg-cream/50 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-ink font-mono">{{ $role->id }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-semibold text-ink">{{ $role->name }}</div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="text-sm text-ink">
                                                ...
                                                @else
                                                    <span class="text-ink/40">No permissions</span>
                                                @endif
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-3">
                                                <a href="{{ route('roles.edit', $role) }}" class="text-accent hover:text-accentlight font-semibold transition-colors inline-flex items-center gap-1">
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center gap-3">
                                                ...
                                                <p class="text-ink/60 text-lg">No roles registered.</p>
                                                <a href="{{ route('roles.create') }}" class="btn-accent text-white font-bold px-6 py-3 rounded-xl inline-flex items-center gap-2 mt-2">
