                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100">
                    <div class="p-6 sm:p-8">
                        <form action="{{ route('blogs.update', $blog) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <!-- Slug -->
                            <div class="mb-6">
                                <label for="slug" class="block text-sm font-bold text-ink mb-2">
                                ...
                                <input type="text"
                                       ...
                                       class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-accent focus:border-accent outline-none transition-all bg-white text-ink font-mono"
                                       required>
                                <p class="mt-2 text-sm text-ink/60">Friendly URL (e.g: my-blog-post)</p>
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <textarea id="content"
                                          name="content"
                                          rows="10"
                                          class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-accent focus:border-accent outline-none transition-all bg-white text-ink resize-none">{{ old('content', $blog->content) }}</textarea>
                            ...
                            @if($blog->image)
                                <div class="mb-6">
                                    ...
                                    <div class="flex items-center space-x-4 p-4 bg-cream rounded-xl border-2 border-gray-200">
                                        <img src="{{ asset('storage/' . $blog->image) }}"
                                             alt="{{ $blog->title }}"
                                             class="h-32 w-auto rounded-xl border-2 border-gray-300 shadow-sm">
                                        <div>
                                            <p class="text-sm text-ink font-semibold">{{ basename($blog->image) }}</p>
                                            <p class="text-xs text-ink/60 mt-1">Upload a new image to replace it</p>
                                        </div>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="mb-6">
                                <label class="flex items-center cursor-pointer group">
                                    ...
                                    <span class="ml-3 text-sm font-semibold text-ink group-hover:text-accent transition-colors">Published</span>
                                </label>
                            </div>
                            ...
                            <div class="flex justify-end space-x-4 pt-4 border-t border-gray-200">
                                ...
                                <button type="submit" class="btn-accent text-white font-bold px-8 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all inline-flex items-center gap-2">
                                    <span class="material-symbols-outlined">save</span> Update Blog
                                </button>
                            </div>
                        </form>
                    </div>
                        ...
                        <div class="preview-container">
                            ...
                            <div class="bg-white border-2 border-gray-200 rounded-2xl shadow-lg overflow-hidden card-hover">
                                ...
                                <div class="relative h-48 sm:h-56 overflow-hidden bg-gray-100">
                                    <img id="preview-image" src="{{ $blog->image ? asset('storage/' . $blog->image) : '' }}" alt="Preview" class="w-full h-full object-cover {{ $blog->image ? '' : 'hidden' }}">
                                    <div id="preview-image-placeholder" class="w-full h-full flex items-center justify-center text-gray-400 {{ $blog->image ? 'hidden' : '' }}">
                                ...
                                <div class="p-6 flex flex-col flex-grow">
                                    <!-- Date Preview -->
                                    <div class="mb-3">
                                        <span id="preview-date" class="text-xs text-ink/60 font-medium">
    ...
    <script>
        ...
        imageInput.addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                if (!currentImageSrc || currentImageSrc.includes('data:')) {
